...
const CASTLE_BRICK = preload("res://stages/world_1/cutscene/CastleBrickEffect.tscn")
const CASTLE_SMOKE = preload("res://stages/world_1/cutscene/CastleSmokeEffect.tscn")
...
func _ready() -> void:
	...
	player.sprite_container.modulate.a = 0.0
	player.completed = true
	await get_tree().create_timer(0.5, false).timeout
	var tw = create_tween()
	tw.tween_property(player.sprite_container, "modulate:a", 1.0, 1.0)
	await get_tree().create_timer(0.5, false).timeout
	_moving = true
	await get_tree().create_timer(2.5, false).timeout
	Audio.play_1d_sound(preload("res://stages/world_1/cutscene/castle_cutscene2.wav"))
	Thunder._current_camera.shock(2, Vector2(4, 4))
	await get_tree().create_timer(2, false).timeout
	_destroying = true
	run_while(
		func():
			Audio.play_1d_sound(preload("res://stages/world_1/cutscene/castle_cutscene.wav"), true, {volume = -6}),
		0.099
	)
